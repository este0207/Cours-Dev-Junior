...
- **Puces** : Supprimez les puces par défaut de la liste `#menu`.
...
- **Alternance** : Dans la section `#cards-container`, ciblez les `.card`. Coloriez le bord gauche de la 1ère et de la 3ème carte en bleu, et la 2ème en orange en utilisant `:nth-child()`.

- **Sélection précise** : Ciblez le deuxième lien du menu sans utiliser de classe, uniquement avec un sélecteur de position.

- **Réflexion** : Si j'ajoute un titre `<h2>` au tout début de ma section `#cards-container`, est-ce que `.card:nth-child(1)` ciblera toujours la première carte ? Pourquoi ? (Répondez en commentaire).


## Exercice 3 : Flexbox - Alignement et Menu

- **Menu Horizontal** : Transformez la balise `ul#menu` en conteneur Flexbox pour aligner les éléments horizontalement.

- **Répartition** : Utilisez `justify-content` pour centrer le menu dans l'en-tête.

- **Boutons** : Ciblez la `div.cta` dans le Hero. Utilisez Flexbox pour aligner les deux boutons côte à côte avec un espace (gap) de 15px.


## Exercice 4 : Grid - La Grille de Cours

- **La Grille** : Ciblez la section `#cards-container`. Transformez-la en grille (grid) avec 3 colonnes de largeur égale en utilisant l'unité `1fr`.
...
- **Exclusion** : Donnez une opacité de 0.6 à toutes les cartes, SAUF celle qui a le statut `.featured"`, en utilisant la pseudo-classe `:not()`.
...
- **Formulaire** : Ciblez les inputs qui sont `required` (obligatoire) et donne leurs une petite bordure rouge à gauche pour les signaler à l'utilisateur.
